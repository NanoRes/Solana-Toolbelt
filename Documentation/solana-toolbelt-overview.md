...
- **Creator tooling for UGC** – `LevelEditorMintService` serialises level data, uploads metadata, and emits registry-ready mint requests, while `SolanaNFTMintService` mints in a per-NFT loop (one transaction per mint) with rent lookups and memo support, refreshing blockhashes on submission failures and relying on the wallet/RPC stack for send behavior.
