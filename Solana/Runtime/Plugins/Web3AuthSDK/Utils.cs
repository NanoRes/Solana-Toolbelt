...
#elif UNITY_ANDROID
        var activity = UnityActivityHelper.GetCurrentActivity();
        if (activity == null)
        {
            Debug.LogWarning("[Web3AuthSDK] Unable to resolve current Android activity. Falling back to Application.OpenURL().");
            Application.OpenURL(url);
            return;
        }
