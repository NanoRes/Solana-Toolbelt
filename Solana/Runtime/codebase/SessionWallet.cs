...
namespace Solana.Unity.SDK
{
    public class SessionWallet : InGameWallet
    {
        ...
        public static PublicKey ExternalAuthorityPublicKey => _externalWallet?.Account?.PublicKey;
        ...
        protected override async Task<Transaction> _SignTransaction(Transaction transaction)
        {
            ...
            var sessionSignedTransaction = await base._SignTransaction(transaction).ConfigureAwait(false);

            if (!RequiresExternalSignature(sessionSignedTransaction))
            {
                return sessionSignedTransaction;
            }

            var sessionSignatures = CloneSignatures(sessionSignedTransaction.Signatures);

            var externallySignedTransaction =
                await _externalWallet.SignTransaction(sessionSignedTransaction).ConfigureAwait(false);

            return MergeTransactionSignatures(
        ...
        protected override async Task<Transaction[]> _SignAllTransactions(Transaction[] transactions)
        {
            ...
            var sessionSignedTransactions = await base._SignAllTransactions(transactions).ConfigureAwait(false);

            if (_externalWallet?.Account == null || Account?.PublicKey == null)
            {
                return sessionSignedTransactions;
            }

            for (int i = 0; i < sessionSignedTransactions.Length; i++)
            {
                var transaction = sessionSignedTransactions[i];
                if (!RequiresExternalSignature(transaction))
                {
                    continue;
                }

                var sessionSignatures = CloneSignatures(transaction.Signatures);

                var externallySignedTransaction =
                    await _externalWallet.SignTransaction(transaction).ConfigureAwait(false);

                sessionSignedTransactions[i] = MergeTransactionSignatures(
                        transaction,
                        sessionSignatures,
                        externallySignedTransaction)
                    ?? transaction;
        ...
        private bool RequiresExternalSignature(Transaction transaction)
        {
            ...
            var sessionPublicKey = Account?.PublicKey;
            if (sessionPublicKey == null || _externalWallet?.Account == null)
            {
                return false;
            }

            foreach (var signaturePair in transaction.Signatures)
            {
                if (signaturePair?.PublicKey == null)
                {
                    continue;
                }

                if (signaturePair.PublicKey.Equals(sessionPublicKey))
                {
                    continue;
                }

                if (IsSignatureEmpty(signaturePair.Signature))
                {
                    return true;
                }
            }
        ...
        private static bool IsSignatureEmpty(byte[] signature)
        {
        ...
        private static List<SignaturePubKeyPair> CloneSignatures(IEnumerable<SignaturePubKeyPair> signatures)
        {
            if (signatures == null)
            {
                return null;
            ...
            var clones = new List<SignaturePubKeyPair>();
            ...
            foreach (var signature in signatures)
            {
                ...
                clones.Add(new SignaturePubKeyPair
                {
        ...
        private static Transaction MergeTransactionSignatures(
            Transaction fallbackTransaction,
            IReadOnlyCollection<SignaturePubKeyPair> originalSessionSignatures,
            Transaction externallySignedTransaction)
        {
            var transaction = externallySignedTransaction ?? fallbackTransaction;
            ...
            if (originalSessionSignatures == null || originalSessionSignatures.Count == 0)
            {
                return transaction;
            }
            ...
            if (transaction.Signatures == null)
            {
                transaction.Signatures = new List<SignaturePubKeyPair>(originalSessionSignatures);
                return transaction;
            }

            var mergedSignatures = new Dictionary<string, SignaturePubKeyPair>(StringComparer.Ordinal);
            ...
            foreach (var signature in transaction.Signatures)
            {
                ...
                mergedSignatures[signature.PublicKey.Key] = signature;
            }

            foreach (var signature in originalSessionSignatures)
            {
                if (signature?.PublicKey == null)
                {
                    continue;
                ...
                if (!mergedSignatures.TryGetValue(signature.PublicKey.Key, out var existingSignature) ||
                    IsSignatureEmpty(existingSignature.Signature))
                {
                    mergedSignatures[signature.PublicKey.Key] = signature;
                }
            }

            transaction.Signatures = mergedSignatures.Values.ToList();
