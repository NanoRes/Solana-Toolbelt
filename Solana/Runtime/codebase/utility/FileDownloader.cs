...
namespace Solana.Unity.SDK.Utility
{
    ...
    public static class FileLoader
    {
        public static async Task<T> LoadFile<T>(string path, string optionalName = "")
        {
            ...
            {
                var source = string.IsNullOrEmpty(optionalName) ? string.Empty : $" for {optionalName}";
                Debug.LogWarning($"[FileLoader] LoadFile<{typeof(T).Name}> received an empty path{source}. Skipping download.");
                return default;
            }
