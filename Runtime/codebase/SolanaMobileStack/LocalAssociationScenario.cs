...
{
    private const int DefaultActivityRetryDelayMs = 200;
    private const int DefaultActivityRetryAttempts = 10;
    ...
    private LocalAssociationScenario(AndroidJavaObject currentActivity, int clientTimeoutMs)
    {
    ...
    public static async Task<LocalAssociationScenario> CreateAsync(
        int clientTimeoutMs = 9000,
        int retryDelayMs = DefaultActivityRetryDelayMs,
        int maxRetryAttempts = DefaultActivityRetryAttempts)
    {
#if UNITY_ANDROID && !UNITY_EDITOR
        if (retryDelayMs <= 0)
        {
            retryDelayMs = DefaultActivityRetryDelayMs;
        }

        if (maxRetryAttempts <= 0)
        {
            maxRetryAttempts = DefaultActivityRetryAttempts;
        }

        AndroidJavaObject activity = null;
        int attemptsRemaining = maxRetryAttempts;
        while (activity == null && attemptsRemaining-- > 0)
        {
            activity = UnityActivityHelper.GetCurrentActivity();
            if (activity != null)
            {
                break;
            }
        ...
        if (activity == null)
        {
            const string errorMessage = "Unable to locate the current Android activity. Ensure the UnityPlayerActivity is available before starting a local association.";
            Debug.LogError($"[LocalAssociationScenario] {errorMessage}");
            throw new InvalidOperationException(errorMessage);
        }

        return new LocalAssociationScenario(activity, clientTimeoutMs);
#else
        const string errorMessage = "Local association requires an active Android activity. This scenario can only be executed on an Android device.";
        Debug.LogError($"[LocalAssociationScenario] {errorMessage}");
